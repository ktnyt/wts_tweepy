...
if __name__ == '__main__':
    ...
    # 自分宛てのツイートを取得して表示
    count = 3
    print '[ Mentions Timeline (あなた宛のツイート) : 最新 {0} 件 ]'.format(count)
    timeline = api.mentions_timeline(count=count)
    for tweet in timeline:
        print '--'
        print '発 言 者:\t{0}'.format(tweet.user.screen_name)
        print '本 　 文:\t{0}'.format(tweet.text.encode('utf-8'))
        print 'MssageId:\t{0}'.format(tweet.id)
        print '発言時間:\t{0}'.format(tweet.created_at)
